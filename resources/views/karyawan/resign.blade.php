<x-app-layout>
    ...
    <div class="py-0 sm:py-6 sm:px-6 lg:px-8">
        ...
        {{-- ── Data Table ──────────────────────────────────────────── --}}
        <x-data-table :paginator="$resigns">
            {{-- Action buttons --}}
            <x-slot name="actions">
            ...
            <x-slot name="thead">
                ...
                <x-table-th>
                    <div class="flex flex-col gap-1.5 py-0.5">
                        <span>NIK</span>
                        <div class="relative">
                            <input type="text" id="filterNIK" value="{{ request('nik') }}"
                                placeholder="Cari..."
                                class="w-full pl-7 pr-2 py-1 text-xs font-normal rounded-md border border-teal-500/40 bg-teal-700/30 dark:bg-teal-900/40 text-white placeholder-teal-200/60 focus:outline-none focus:ring-1 focus:ring-white/40 focus:border-white/40 transition">
                            <svg class="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-teal-200/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M17 11A6 6 0 105 11a6 6 0 0012 0z"/>
                ...
                <x-table-th>
                    <div class="flex flex-col gap-1.5 py-0.5">
                        <span>Jabatan</span>
                        <div class="relative">
                            <input type="text" id="filterJabatan" value="{{ request('jabatan') }}"
                                placeholder="Cari..."
                                class="w-full pl-7 pr-2 py-1 text-xs font-normal rounded-md border border-teal-500/40 bg-teal-700/30 dark:bg-teal-900/40 text-white placeholder-teal-200/60 focus:outline-none focus:ring-1 focus:ring-white/40 focus:border-white/40 transition">
                            <svg class="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-teal-200/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M17 11A6 6 0 105 11a6 6 0 0012 0z"/>
                ...
                <x-table-th>
                    <div class="flex flex-col gap-1.5 py-0.5">
                        ...
                        <div class="relative">
                            <input type="text" id="filterDepartemen" value="{{ request('departemen') }}"
                                placeholder="Cari..."
                                class="w-full pl-7 pr-2 py-1 text-xs font-normal rounded-md border border-teal-500/40 bg-teal-700/30 dark:bg-teal-900/40 text-white placeholder-teal-200/60 focus:outline-none focus:ring-1 focus:ring-white/40 focus:border-white/40 transition">
                            <svg class="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-teal-200/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M17 11A6 6 0 105 11a6 6 0 0012 0z"/>
                ...
                <x-table-th>
                    <div class="flex flex-col gap-1.5 py-0.5">
                        <span>Instalasi / Devisi</span>
                        <div class="relative">
                            <input type="text" id="filterUnit" value="{{ request('unit') }}"
                                placeholder="Cari..."
                                class="w-full pl-7 pr-2 py-1 text-xs font-normal rounded-md border border-teal-500/40 bg-teal-700/30 dark:bg-teal-900/40 text-white placeholder-teal-200/60 focus:outline-none focus:ring-1 focus:ring-white/40 focus:border-white/40 transition">
                            <svg class="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-teal-200/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M17 11A6 6 0 105 11a6 6 0 0012 0z"/>
                ...
                <x-table-th align="center">Tanggal Resign</x-table-th>
                <x-table-th align="center">Action</x-table-th>
            </x-slot>

            {{-- Table body --}}
            <x-slot name="tbody">
                @forelse ($resigns as $karyawan)
                <tr class="odd:bg-white even:bg-gray-50 dark:odd:bg-gray-900/50 dark:even:bg-gray-950 border-t border-gray-100 dark:border-gray-800 hover:bg-teal-50/40 dark:hover:bg-teal-900/10 transition-colors duration-100 table-row">
                    <td class="px-5 py-3.5 text-gray-500 dark:text-gray-400 tabular-nums text-sm">
                        {{ ($resigns->currentPage() - 1) * $resigns->perPage() + $loop->iteration }}
                    </td>
                    <td class="px-5 py-3.5 font-medium text-gray-800 dark:text-gray-200">
                        {{ $karyawan->name }}
                    </td>
                    <td class="px-5 py-3.5 text-gray-800 dark:text-gray-200">
                        {{ $karyawan->nik }}
                    </td>
                    <td class="px-5 py-3.5 text-gray-800 dark:text-gray-200">
                    ...
                    <td class="px-5 py-3.5 text-gray-800 dark:text-gray-200 text-center">
                        {{ $karyawan->tgl_resign ? \Carbon\Carbon::parse($karyawan->tgl_resign)->translatedFormat('d F Y') : '-' }}
                    </td>
                    <td class="px-5 py-3.5 text-right space-x-1">
                        ...
                        <button x-data
                            @click="$dispatch('open-modal-hapus', { id: {{ $karyawan->id }}, name: '{{ addslashes($karyawan->name) }}' })"
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-5 py-14 text-center text-gray-400 dark:text-gray-500">
    ...
    {{-- Modal Hapus --}}
    <x-modal-hapus entity="karyawan" :deleteUrl="url('karyawan')" />
    ...
    <script>
        ...
        const filters = [
            ...
            { id: 'filterNIK',        param: 'nik',        type: 'text' },
            { id: 'filterJabatan',    param: 'jabatan',    type: 'text' },
            { id: 'filterDepartemen', param: 'departemen', type: 'text' },
            { id: 'filterUnit',       param: 'unit',       type: 'text' },
        ]
            ...
            filters.forEach(({ id, param, type }) => {
                ...
                if (type === 'select') {
                    ...
                    if (val.length === 0) {
                        ...
                        // Karakter "-" langsung diproses tanpa minimal panjang
                        url.searchParams.set(param, val)
                        shouldFetch = true
                    } else if (val.length >= 2 && val !== current) {
                        url.searchParams.set(param, val)
                        shouldFetch = true
                    }
                }
